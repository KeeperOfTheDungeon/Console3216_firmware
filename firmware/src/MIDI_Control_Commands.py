...
class MIDI_Control_Commands:
    ...
    # Hilfsmethode
    @classmethod
    def serialWriteData(cls, command: int, trackId: int, tempo: int = -1):
        byteConverter: byteConverter
        byteConverter.value = trackId

        firstByteTrackId = byteConverter.firstByte
        secondByteTrackId = byteConverter.secondByte

        if tempo != -1:
            byteConverter.value = tempo
            firstByteTempo = byteConverter.firstByte
            secondByteTempo = byteConverter.secondByte
    ...
    def playTrack(cls, trackId: int, tempo: int) -> int:
        return cls.serialWriteData(cls.__COMMAND_CODE_PLAY_TRACK, trackId, tempo)

    @classmethod
    def changeTempo(cls, trackId: int, tempo: int) -> int:
        return cls.serialWriteData(cls.__COMMAND_CODE_CHANGE_TEMPO, trackId, tempo)

    @classmethod
    def pauseTrack(cls, trackId: int) -> int:
        return cls.serialWriteData(cls.__COMMAND_CODE_PAUSE_TRACK, trackId)

    @classmethod
    def stopTrack(cls, trackId: int) -> int:
        return cls.serialWriteData(cls.__COMMAND_CODE_STOP_TRACK, trackId)

    @classmethod
    def resumeTrack(cls, trackId: int) -> int:
        return cls.serialWriteData(cls.__COMMAND_CODE_RESUME_TRACK, trackId)

    @classmethod
    def restartTrack(cls, trackId: int) -> int:
        return cls.serialWriteData(cls.__COMMAND_CODE_RESTART_TRACK, trackId)
